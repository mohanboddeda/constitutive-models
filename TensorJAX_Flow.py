...
TensorJAX_Flow.py
-----------------
Unified training script for FLOW-SPECIFIC data (Maxwell-B focus).
Mirrors the logic of TensorJAX_Uniform.py but adapted for:
- Iterating over specific flow types (Uniaxial, Shear, etc.).
- Flow-specific directory structure.
- Flow-specific pre/post-processing utilities.
"""
...
# --- Import Custom Utilities (FLOW Versions) ---
# Pre-train: Data Loading & Checkpointing
from utils.pretrain_flow import (
    load_and_normalize_flow_data_replay, 
    save_checkpoint, 
    load_checkpoint
)
...
#============================================================
# 8. Main training function (Per Stage, Per Flow)
#============================================================
def run_training_stage(cfg, flow_type, stage_tag, data_tuple, output_dir, transfer_params=None):
    ...
    # 1. Folder Setup
    # Path: trained_models/flowdata/.../flow_type/maxwell_B_STAGE
    ...
    threshold = 0.1 
    significant_mask = np.abs(y_true_phys) > threshold
    if np.sum(significant_mask) > 0:
        raw_rel_error = np.abs(y_true_phys - y_pred_phys) / (np.abs(y_true_phys) + 1e-8)
        filtered_mre = np.mean(raw_rel_error[significant_mask]) * 100.0
        print(f"\n✅ CORRECTED MRE (excluding values < {threshold}): {filtered_mre:.2f}%")
    else:
        filtered_mre = 0.0
        print(f"\n⚠️ All test values are below threshold {threshold}. Cannot compute MRE.")
    ...
    metrics_table = [
        ...
        ["Test/MSE", test_mse],
        ["Test/MAE", test_mae],
        ["Test/Filtered_MRE_%", filtered_mre]
    ]

    # 4. Save Summary & Table to SPECIFIC Run Folder
    shared_metrics_dir = output_dir 
    my_log_name = f"{cfg.model_type}_{cfg.mode}_{flow_type}_metrics.txt"
...
#============================================================
@hydra.main(config_path="config/train", config_name="flow_tensor_config", version_base=None)
def main(cfg: DictConfig):
    
    total_start_time = time.time()

    print(f"\n{'='*60}")
    print(f"🚀 STARTING FLOW TRAINING | Mode: {cfg.mode.upper()} | Model: {cfg.model_type}")
    print(f"🌊 Flows to Train: {cfg.flow_types}")
    print(f"{'='*60}\n")

    # Define Size Folder String
    ...
    for flow_type in cfg.flow_types:
        try:
            ...
            # 1. Output Path: trained_models/flowdata/{mode}/seed_XX/{flow_type}/{size_folder}
            if cfg.mode in ["single_stage", "multi_stage"]:
                base_out = os.path.join("trained_models", "flowdata", cfg.mode, f"seed_{cfg.seed}", flow_type, size_folder)
            else:
                raise ValueError(f"Invalid mode: {cfg.mode}")
            
            os.makedirs(base_out, exist_ok=True)

            # 2. Load Data for this SPECIFIC Flow Type
            # Note: This function handles the stages internal to that flow type
            data_stages = load_and_normalize_flow_data_replay(
                flow_type=flow_type,
                model_type=cfg.model_type,
                data_root="datafiles", 
                mode=cfg.mode,
