...
def main(cfg: DictConfig) -> None:
    ...
    Logic:
    1. Define Stages exactly as requested (including discrete 1.0).
    2. Generate GLOBAL pool of candidates via Uniform Rate Sampling.
    3. Sort (Bin) candidates into the correct stage based on Condition Number.
    4. Filter for Admissible Region (Lumley Triangle).
    5. Save to folder structure: 
       flowdata/<mode>/seed_<seed>/<stage>/<flow_type>/<size_folder>/
    """
    ...
    for flow_type in cfg.flow_types:
        ...
        print(f"   -> Generating {cfg.n_samples} candidates (distributing into {len(bin_keys)} bins)...")
        ...
        for stage_tag, items in data_bins.items():
            ...
            if len(L_final) > 0:
                ...
                # =========================================================
                # DEFINE SIZE FOLDER (NEW LOGIC)
                # =========================================================
                # This matches the logic from generateUniformMaxwell.py
                if cfg.n_samples >= 1000:
                    size_folder = f"{int(cfg.n_samples/1000)}ksamples"
                else:
                    size_folder = f"{cfg.n_samples}samples"

                # G. Construct Paths (Hierarchy: Mode / Seed / Stage / Flow / Size)
                # Added 'size_folder' to the path structure here
                base_suffix = os.path.join("flowdata", folder_mode, f"seed_{cfg.seed}", stage_tag, flow_type, size_folder)
                ...
                print(f"      ✅ Saved {len(L_final)} samples to {stage_tag}/{flow_type}/{size_folder}")
    ...
    for st in bin_keys:
        # Define root folder for this stage
        data_root_stage = os.path.join(cfg.paths.data, "flowdata", folder_mode, f"seed_{cfg.seed}", st)
        images_root_stage = os.path.join(cfg.paths.images, "flowdata", folder_mode, f"seed_{cfg.seed}", st)
