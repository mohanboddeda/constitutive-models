...
#from utils.curriculum_loader import load_curriculum_or_stageonly
...
def run_training_for_lambda(cfg, lambda_val,
    ...
    out_dir = os.path.join(cfg.output_dir,
                           f"{cfg.model_type}_stage_{stage_tag}_lambda_{lambda_val}")
    ...
    if transfer_ckpt_path:
        ...
        # Initialize dummy parameters first
        ...
        print(f"🛡️ Transfer Mode: Physics constraints enabled fully (λ={lambda_val}) to prevent forgetting.")
    else:
    ...
    for epoch in range(cfg.training.num_epochs):
        ...
        # --- MODIFIED LAMBDA SCHEDULE ---
        if transfer_ckpt_path:
            # Maintain full physics constraint immediately to prevent drift
            lambda_curr = lambda_val
        else:
            # Linear warmup only for fresh training
        ...
        if epoch % 50 == 0 or epoch == cfg.training.num_epochs - 1:
            print(f"[Stage {stage_tag}] Epoch {epoch}: λ_phys={lambda_curr:.4f}, Val Loss={avg_val_loss:.6e}")
    ...
    # Dataset-wide prediction analysis
    ...
    # Save metrics table to a .txt file
...
def main(cfg: DictConfig):
    ...
    print(f"=== Stage {stage_tag} ({cfg.model_type}) ===")
    ...
    if cfg.transfer_checkpoint is None or cfg.transfer_checkpoint == "null":
       ...
       print(f"🔄 Using transfer checkpoint from: {transfer_ckpt}")
    
    run_training_for_lambda(cfg, cfg.training.lambda_phys[0], Xtr, Xv, Xt,
