...
def main(cfg: DictConfig):
    ...
    results = load_and_normalize_stagewise_data_stable(cfg.model_type, "datafiles", stages_list,
                                                       seed=cfg.seed, scaling_mode=cfg.data.scaling_mode)
    transfer_ckpt = None
    for stage_tag in stages_list:
        Xtr, Xv, Xt, Ytr, Yv, Yt, X_mean, X_std, Y_mean, Y_std = results[stage_tag]
        transfer_ckpt = run_training_for_lambda(cfg, cfg.training.lambda_phys[0],
                                                Xtr, Xv, Xt,
                                                Ytr, Yv, Yt,
                                                X_mean, X_std, Y_mean, Y_std,
                                                stage_tag,
                                                transfer_ckpt_path=transfer_ckpt)
