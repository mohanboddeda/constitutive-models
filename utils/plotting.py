...
def analyze_and_plot_dataset(X_flat, Y_flat, cfg):
    ...
    saved into per-model subfolders inside analysis_output.
    """
    ...
    # Stability suffix
    stability_str = "stable" if cfg.stable else "unstable"
    run_id = f"{cfg.constitutive_eq}_{stability_str}"

    # Create per-model folder automatically
    model_folder_name = cfg.constitutive_eq  # like "maxwell_B", "oldroyd_B", "ptt_exponential"
    analysis_dir_base = os.path.join(cfg.paths.images, "analysis_output")

    # Create stability subfolder inside model_dir
    analysis_dir = os.path.join(analysis_dir_base, model_folder_name, stability_str)
    os.makedirs(analysis_dir, exist_ok=True)
    print(f"📂 Saving analysis output to: {analysis_dir}")

    # Each model's stats text file – separate
    ...
    with open(stats_file_path, "a") as f:
        ...
        def _process_variable(data_flat, var_name, var_type):
            ...
            # Write stats to model-specific text file
            f.write(f"--- {var_name} ---\n")
