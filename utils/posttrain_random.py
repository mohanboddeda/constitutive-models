...
def plot_global_stress_summary(y_true, y_pred, fig_dir, model_type):
    ...
    plt.suptitle(f"{clean_model} (Random): Global Test Set Summary", fontsize=15, y=0.98)
    plt.tight_layout()
...
def plot_dataset_predictions_summary(y_true_phys, y_pred_phys, fig_dir, shared_log_dir, 
    ...
    if log_filename is None: fname = f"{model_type}_metrics.txt"
    ...
    with open(metrics_path, "a", encoding="utf-8") as f:
        f.write("\n" + "="*80 + "\n")
        # --- UPDATED HEADER WITH TIME ---
        f.write(f" RUN SUMMARY | Model: {model_type} | Time: {time_str} | Seed: {seed} | Size: {n_samples} | Date: {np.datetime64('now')}\n")
        f.write("="*80 + "\n\n")
    ...
    plt.title("Maxwell B - True vs Predicted ($T_{{ij}}$)")
    plt.grid(True, ls="--")
    plt.tight_layout()
...
def run_post_training_analysis(
    ...
    # (Removed single sample analysis and residual hist as requested)
