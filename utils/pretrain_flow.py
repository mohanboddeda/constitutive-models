#!/usr/bin/env python3
"""
Pre-training Data Loader & Statistics Generator (Flow Types)
Responsible for: 
...
def plot_invariants_replay_mode(X_curr, X_replay, stage_tag, save_dir, flow_type):
    ...
    # Handle potentially empty replay buffer
    ...
    n_curr = len(X_curr)
    n_replay = len(X_replay) if X_replay is not None else 0
    ...
    legend_elements = [
        ...
        # Metadata
        Line2D([0], [0], color='none', label=f'Flow: {clean_flow}'),
    ]
...
# ===========================================================
# 2. Stage-wise Replay Loader (Flow Logic)
# ===========================================================
def load_and_normalize_flow_data_replay(
    ...
    for stage_tag in stage_order:
        ...
        if mode == "single_stage":
            ...
            print(f"\n=== Stage {stage_tag} ({flow_type}) - REPLAY MODE (Ratio={replay_ratio}) ===")
        
        # --- PATH CONSTRUCTION (Logic from generateFlowMaxwell.py) ---
        # Note: We must replicate the 'size_folder' logic to find the files
        if n_samples >= 1000:
            size_folder = f"{int(n_samples/1000)}ksamples"
        else:
            size_folder = f"{n_samples}samples"

        # Path: datafiles/flowdata/<mode>/seed_<seed>/<stage>/<flow_type>/<size_folder>/
        stage_dir = os.path.join(data_root, "flowdata", mode, f"seed_{seed}", stage_tag, flow_type, size_folder)
        ...
        norm_dir = os.path.join("images", "flowdata", mode, f"seed_{seed}", stage_tag, flow_type, size_folder, "normalized")  
        ...
        if mode == "multi_stage" and stage_tag in stage_order and stage_order.index(stage_tag) > 0 and replay_ratio > 0:
            ...
            for prev_stage in stage_order[:current_idx]:
                prev_dir = os.path.join(data_root, "flowdata", mode, f"seed_{seed}", prev_stage, flow_type, size_folder)
        ...
        # --- VISUALIZATION STEP ---
        # Plot the Invariant Space showing Current (Red) vs Replay (Blue)
        # MODIFICATION: Only run this plot if we are in multi_stage mode
        if mode == "multi_stage":
            try:
                print("   🎨 Generating Replay Invariant Diagram...")
                # If we added replay, we slice X to separate current/replay for plotting
                # If no replay, X_curr is just X
        ...
        # 5. Normalization (Calculated on TRAIN only)
        ...
        if scaling_mode == "standard":
            ...
            Y_mean, Y_std = Y_min, Y_range # Pack for consistent return signature
            strategy_name = "MinMax"
        else:
        ...
        print(f"✅ Splitting Strategy: {strategy_name}")
