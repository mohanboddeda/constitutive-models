#!/usr/bin/env python3
"""
Pre-training Data Loader & Statistics Generator (Uniform Distribution)
Responsible for: 
1. Loading Stage-wise Data (Single/Multi-stage) from Uniform Grid Generation.
...
def load_and_normalize_stagewise_data_replay(
    ...
    for stage_tag in stage_order:
        ...
        # --- HEADER LOGGING ---
        if mode == "single_stage":
            print(f"\n=== Single Stage Training: {model_type} (Uniform) ===")
        else:
            print(f"\n=== Stage {stage_tag} ({model_type}) - REPLAY MODE (Ratio={replay_ratio}) ===")
        
        # --- PATH CONSTRUCTION ---
        # NOTE: uniform generator saves directly to 'seed_{seed}/{stage_tag}/' 
        # without an intermediate '{n}samples' folder.
        if n_samples >= 1000:
        ...
        # We assume 'images/uniform' for uniform plots
