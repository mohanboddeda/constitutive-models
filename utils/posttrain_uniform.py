#!/usr/bin/env python3
"""
Post-Training Analysis Utilities (Uniform Distribution)
...
def plot_all_losses(train_d, val_d, train_p, val_p, Y_std, fig_dir, model_type, stage_tag, n_samples=None):
    ...
    plt.title("Training & Validation Loss Curves (Uniform)", fontsize=16, pad=12)
...
# ===========================================================
def plot_global_stress_summary(y_true, y_pred, fig_dir, model_type):
    """
    ...
    # --- UPDATED: Engineering Floor Logic (Matches Text Report) ---
    # We use a floor of 0.005 to match the logic used in plot_dataset_predictions_summary
    FLOOR = 0.005
    ...
    plots_config = [
        ...
        {
            "data": mean_rel_err,
            "title": f"Mean Relative Error %", # Updated Title
            "cmap": "magma", "fmt": "{:.1f}%",
    ...
    plt.suptitle(f"{clean_model} (Uniform): Global Test Set Summary", fontsize=15, y=0.98)
    plt.tight_layout()
...
def plot_dataset_predictions_summary(y_true_phys, y_pred_phys, fig_dir, shared_log_dir, 
                                     ...
                                     stage_tag="Unknown", elapsed_time=None): # <--- Added Arg
    """
    1. Saves Plots (Histograms, Scatter) to the specific run folder (fig_dir).
    2. Appends Text Metrics to a central file.
    Updated with Time Logging and Engineering Floor logic.
    """
    os.makedirs(fig_dir, exist_ok=True)
    ...
    if log_filename is None: fname = f"{model_type}_uniform_metrics.txt"
    else: fname = log_filename
    metrics_path = os.path.join(shared_log_dir, fname)

    # --- CALC NEW METRICS (Global) ---
    ...
    with open(metrics_path, "a", encoding="utf-8") as f:
        f.write("\n" + "="*80 + "\n")
        f.write(f" RUN SUMMARY | Model: {model_type} (Uniform) | Time: {time_str} | Seed: {seed} | Size: {n_samples} | Date: {np.datetime64('now')}\n")
        f.write("="*80 + "\n\n")
    ...
    plt.suptitle(f"{clean_model} (Uniform): Component-wise Error Distribution ($T_{{ij}}$)", fontsize=16, y=0.98)
    plt.tight_layout()
    ...
    plt.title(f"{clean_model} (Uniform) - True vs Predicted ($T_{{ij}}$)")
    plt.grid(True, ls="--")
    plt.tight_layout()
...
def run_post_training_analysis(
    y_true_n, y_pred_n, 
    Y_mean, Y_std, 
    stage_tag, model_type, base_dir, n_samples=None
):
    """
    Standard entry point for validation (Uniform).
    ...
    try:
        plot_global_stress_summary(y_true, y_pred, fig_dir, model_type)
...
if __name__ == "__main__":
    print("Post-training analysis module loaded (Uniform).")