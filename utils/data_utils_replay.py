...
# ===========================================================
# 1. Stage-wise Replay Loader
# ===========================================================
def load_and_normalize_stagewise_data_replay(
    model_type, data_root, stages,
    seed=42, scaling_mode="standard", replay_ratio=0.2
):
    """
    Stage-wise loader with experience replay.
    For later stages, mixes in a fraction (`replay_ratio`) of data from previous stages.
    """
    results = {}
    ...
    for stage_tag in stages:
        ...
        # Experience replay for later stages
        if stage_tag in stage_order and stage_order.index(stage_tag) > 0 and replay_ratio > 0:
            ...
            for prev_stage in stage_order[:current_idx]:
                pX_path = os.path.join(data_root, prev_stage, f"X_3D_{model_type}_stable.pt")
                pY_path = os.path.join(data_root, prev_stage, f"Y_3D_{model_type}_stable.pt")
            ...
            if replay_X_list:
                ...
                if all_prev_X.shape[0] >= num_replay:
                    ...
                    print(f"   ⚠️ Not enough replay samples, using all")
        ...
        if scaling_mode == "standard":
            ...
            raise ValueError("Invalid scaling_mode")

        # Save stats & plots
        norm_dir = os.path.join("images", f"{stage_tag}_replay", model_type, "normalized")
        os.makedirs(norm_dir, exist_ok=True)

        def write_stats(f, arr, label):
            flat = arr.flatten()
            f.write(f"--- {label} ---\n")
            f.write(f"Mean:  {np.mean(flat):.4e}\n")
            f.write(f"Std:   {np.std(flat):.4e}\n")
            f.write(f"Min:   {np.min(flat):.4e}\n")
            f.write(f"Max:   {np.max(flat):.4e}\n\n")

        with open(os.path.join(norm_dir, "normalizeddatastat.txt"), "w") as f:
            f.write(f"=== Stats for {model_type} Stage {stage_tag} (Replay) ===\n")
            write_stats(f, X_train_n, "X_train Norm")
            write_stats(f, Y_train_n, "Y_train Norm")

        def plot_hist(X_data, Y_data, set_name):
            fig, axes = plt.subplots(1, 2, figsize=(12, 6))
            axes[0].hist(X_data.flatten(), bins=50, color='steelblue', log=True)
            axes[1].hist(Y_data.flatten(), bins=50, color='orange', log=True)
            axes[0].set_title(f"L ({set_name})")
            axes[1].set_title(f"T ({set_name})")
            plt.tight_layout()
            plt.savefig(os.path.join(norm_dir, f"hist_{set_name}.png"))
            plt.close()

        plot_hist(X_train_n, Y_train_n, "Train")
